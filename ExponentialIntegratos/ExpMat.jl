...
struct ExpMatrix{T <: Number}
    basis :: Matrix{T}
    order :: Integer
    function ExpMatrix{T}(mat::Matrix{T}; order::Integer) where T <: Number
...
import Base.*
"""
    *(expmat::ExpMatrix, b::Vector{<:Number}; method::String="Contour-Krylov")
...
"""
function *(expmat::ExpMatrix, b::Vector{<:Number}; method::String="Contour-Krylov")
    ...
    return 1
    # if method == "Krylov"
    #     return krylov(A, b, order)
    # elseif method == "Contour"
    #     return contour(A, b, order)
    # else
    #     return contour_krylov(A, b, order)
    # end
end
